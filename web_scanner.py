...
FILES_DICTIONARY = os.getenv("FILES_DICTIONARY")
...
class WebScanner:
    def __init__(self, target):
        self.target = target
        self.threads = "40"
        self.extensions = "html,php,sh,txt,asp"
    ...
    def cleaner_gobuster(self,output_dict):
        cleaned_results = {}
    ...
    def scanner(self, port_list, type="directories"):
        all_results = {}
        self.wordlist_dic = DIR_DICTIONARY
        self.wordlist_fil = FILES_DICTIONARY
        ...
        for port in port_list:
            ...
            if type == "directories":
                print(f"\nNow scanning {host} for possible directories")
                command.append("-w")
                command.append(self.wordlist_dic)
            else:
                print(f"\nNow scanning {host} for possible files")
                command.append("-w")
                command.append(self.wordlist_fil)
                command.append("-x")
